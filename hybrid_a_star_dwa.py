...
class HybridPlannerNode(Node):
    def __init__(self):
        ...
        self.costmap_size = 100  # 100 x 100 grid cells (e.g., 5x5 meters if resolution is 0.05)
    ...
    def lookup_transform_with_retry(self, target_frame, source_frame, timeout=1.0, retries=3):
        ...
        self.get_logger().error(f"Failed to lookup transform from {source_frame} to {target_frame}.")
        return None
    ...
    def generate_dwa_trajectories(self, current_position, current_yaw, v_range, w_range, dt, horizon):
        trajectories = []
        for v in np.linspace(v_range[0], v_range[1], 50):  # Increase resolution to 30 steps
            for w in np.linspace(w_range[0], w_range[1], 50):  # Increase angular velocity resolution
    ...
    def evaluate_dwa_trajectory(self, trajectory, global_path):
        if not trajectory or not global_path:
        ...
        for point in trajectory:
            # Path alignment cost
            path_dist = min(np.linalg.norm(np.array(point) - np.array(waypoint)) for waypoint in global_path)
            path_cost += path_dist
            ...
            collision_cost += 1.0 / (obs_dist + 1e-6)

        # Penalize rotational motion (use current yaw instead of trajectory yaw)
        if len(trajectory) > 1:
            traj_direction = np.array(trajectory[-1]) - np.array(trajectory[0])
            target_direction = np.array(global_path[0]) - np.array(trajectory[0])
            traj_angle = np.arctan2(traj_direction[1], traj_direction[0])
            target_angle = np.arctan2(target_direction[1], target_direction[0])
            heading_cost = abs(self.normalize_angle(target_angle - traj_angle))

        total_cost = (
            -1.0 * path_cost  # Penalize distance from path
            - 2.0 * collision_cost  # Penalize proximity to obstacles
            - 2.0 * heading_cost  # Penalize heading misalignment
        )
        return total_cost


    def inflate_costmap(self, inflation_radius=0.2):
    ...
    def dwa_local_planner(self, current_position, current_yaw):
        ...
        for trajectory, v, w in candidates:
            if v < 0.05 and abs(w) > 0.5:  # Avoid pure rotational motion
                continue
            angular_velocity_penalty = 0.1 * abs(w)  # Penalize high angular velocity
            score = self.evaluate_dwa_trajectory(trajectory, [target_waypoint]) - angular_velocity_penalty
            if score > best_score:
                best_score = score
    ...
    def control_loop(self):
        ...
        if not self.path:
            self.get_logger().warn("No waypoints available. Stopping the robot.")
            self.cmd_vel_pub.publish(Twist())  # Stop the robot
            return

        target_waypoint = np.array(self.path[0])

        # Compute direction and heading error
        ...
        heading_error = self.normalize_angle(angle_to_waypoint - yaw)

        # If the heading error is significant, prioritize rotation
        if abs(heading_error) > 0.1:  # Adjust threshold for alignment (e.g., 0.1 radians ~ 5.7 degrees)
            twist = Twist()
            twist.angular.z = max(-self.max_angular_speed, min(self.max_angular_speed, 2.0 * heading_error))  # Proportional control
            self.cmd_vel_pub.publish(twist)
            self.get_logger().info(f"Rotating to align with waypoint. Heading error: {heading_error:.2f}")
            return

        # If aligned, move toward the waypoint
        twist = Twist()
        twist.linear.x = min(self.max_linear_speed, 0.5 * distance_to_waypoint)  # Scale speed by distance
        self.cmd_vel_pub.publish(twist)
        self.get_logger().info(f"Moving toward waypoint: linear={twist.linear.x:.2f}")

        # Check if the waypoint is reached and remove it
        ...
        # Visualize the next target waypoint
        if self.path:
    ...
    def get_lookahead_point(self, current_position):
        ...
        for point in self.path:
            if np.linalg.norm(np.array(point) - current_position) >= lookahead_distance:
                return np.array(point)

        # If no lookahead point is found, return the last waypoint
        return np.array(self.path[-1]) if self.path else None
